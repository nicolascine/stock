/**
 * A) Portfolio class that has a collection of Stocks and a "Profit" method that receives 2 dates and
 *    returns the profit of the Portfolio between those dates. Assume each Stock has a "Price" method
 *    that receives a date and returns its price.
 * B) Profit method return the "annualized return" of the portfolio between the given dates.
 */
...
class Portfolio {
  ...
  public getProfit(startDate: string, endDate: string): number {
    const totalStartValue = this.stocks.reduce(
      (sum, stock) => sum + stock.getPrice(startDate),
      0
    );
    const totalEndValue = this.stocks.reduce(
      (sum, stock) => sum + stock.getPrice(endDate),
      0
    );
...
console.log(
  `Profit from ${startDate} to ${endDate}:`,
  portfolio.getProfit(startDate, endDate).toFixed(2)
);
console.log(
  `Annualized Return from ${startDate} to ${endDate}:`,
  (portfolio.getAnnualizedReturn(startDate, endDate) * 100).toFixed(2) + "%"
);
